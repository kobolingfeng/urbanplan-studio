...
- 导入报告：载入 UPF/GeoJSON 后立即展示对象统计、当前方案、兼容修复、归一化补齐项和对应格式的校验发现。
...
- GeoJSON 导入导出：可把当前语义对象导出为 FeatureCollection，并可重新载入带 UPF 属性的 GeoJSON 文件。
...
- 导入经过兼容层处理，缺失字段会补默认值，非法对象会被过滤；GeoJSON/CSV 导入报告会区分适配器审计和 UPF schema 校验。
...
- 删除对象时阻止产生出入口悬挂引用。
- 新增 `smoke:geometry`、`smoke:upf`、`smoke:rules`、`smoke:rule-fixtures`、`smoke:geojson`、`smoke:csv`、`smoke:static`、`smoke:release` 和 `verify` 脚本。
