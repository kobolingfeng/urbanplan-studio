...
- 对象 Inspector：可编辑地块用地、容积率、控制 FAR、建筑密度、绿地率、控制建筑密度、控制绿地率、限高、住宅建面、公服建面、更新方式等，数值输入会按合理范围夹取。
- 结构化证据链：每个对象可维护字符串证据或 EvidenceSource（类型、时间、精度、许可、可信度），影响数据质量诊断和方案可信度。
- 导入审计：载入 UPF 后记录缺失字段、兼容修复和需复核对象，并并入质检报告。
- 质量扣分解释：数据质量报告列出扣分项、数量、单项扣分和总扣分，便于复核评分来源。
- 导入报告：载入 UPF/GeoJSON/CSV 后立即展示对象统计、当前方案、兼容修复、归一化补齐项和对应格式的校验发现。
- UPF 结构校验：检查格式版本、必填字段、对象类型、情景完整性、数值范围、坐标系一致性和出入口引用完整性。
- 规则检查：检查容积率、绿地率、建筑密度、用地兼容、公共服务缺口、道路红线、出入口道路风险、历史风貌区更新风险。
- 规则目录：可独立查看规则分布，以及每条规则的领域、默认等级、结构化 RuleSource、计算口径和是否属于原型启发式判断。
- 智能建议：根据规则问题生成可追溯建议。
- 综合评估：从控规符合性、公共服务、交通组织、生态开放空间、更新价值和证据可信度生成方案评分。
- 服务人口分摊：综合评估报告按地块展示人口、公服建面、幼儿园和养老需求及覆盖状态。
- 需求假设：服务人口、幼儿园学位和养老服务能力采用共享参数，评估报告会显式说明。
- 权重敏感性：用均衡、公共服务优先、保护生态优先、实施风险优先四类模型复核推荐方案是否稳健。
- 决策矩阵：对全部方案重新运行规则和评分，输出推荐方案、风险对照和答辩解释。
- 方案 CSV：可单独导出图表友好的长表指标 CSV，覆盖评分、人口、建面、风险计数和维度得分。
- CSV 指标导入：可载入地块指标表，按 `parcel_id` 和 `scenario_id` 更新情景 FAR、绿地率、建面和备注，并报告跳过行与非法数值。
- 案例验证包：汇总研究问题、数据概况、方案矩阵、敏感性、实验记录表和 CSV 附录。
- 评分热力图：地块颜色随综合评分变化，便于现场识别优先优化对象。
- 对象搜索与风险筛选：可按关键词、问题对象、高风险对象或完整对象类型快速定位。
- 方案优化策略：支持合规回调、公共服务优先、生态保护优先三类快速方案调整。
- 图面解释增强：方案列表直接显示评分，选中公共设施时显示服务半径覆盖圈。
- 键盘工作流：支持对象搜索、对象列表方向键导航、画布平移缩放、运行检查、保存和载入的常用快捷操作。
- UPF 输出：可查看和保存 `.upf` JSON 数据。
- GeoJSON 导入导出：可把当前语义对象导出为 FeatureCollection，并可重新载入带 UPF 属性的 GeoJSON 文件；导入时会补齐当前情景并过滤越界指标。
...
- UPF 保存现在带顶层 `format/formatVersion`，保存后可以再次载入；自家导出的 GeoJSON FeatureCollection 也可回导为 UPF 语义对象。
- 导入经过兼容层处理，缺失字段会补默认值，非法对象会被过滤；GeoJSON/CSV 导入报告会区分适配器审计和 UPF schema 校验，CSV 即使 0 行匹配也会报告跳过行。
...
- 删除对象时阻止产生出入口悬挂引用。
- 新增 `smoke:geometry`、`smoke:upf`、`smoke:rules`、`smoke:rule-fixtures`、`smoke:evaluation`、`smoke:geojson`、`smoke:csv`、`smoke:markdown`、`smoke:evidence`、`smoke:ranges`、`smoke:dev-utils`、`smoke:robustness`、`smoke:static`、`smoke:release` 和 `verify` 脚本。
- 新增 `planning-evaluation.ts`，把多指标方案评价纳入自动验证。
