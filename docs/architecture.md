...
10. Evidence parsing and scoring smoke tests.
11. Shared numeric range smoke tests.
12. Dev utility smoke tests.
13. Robustness smoke tests for malformed public inputs.
14. Full frontend + native build.
15. Static build smoke tests.
...
UPF/demo project
  ...
  -> evaluateScenario()
  -> report / decision matrix / sensitivity analysis / data-quality / UPF and GeoJSON export
```
...
- UPF validation now has a lightweight runtime validator and a draft JSON Schema. Evidence can be legacy strings or structured EvidenceSource objects, and EPSG:4490 imports with canvas-scale coordinates are rejected as mixed CRS data. Full CRS transforms and migration/version negotiation are still future work.
