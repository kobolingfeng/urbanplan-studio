...
- Run `bun run clean`.
- Run `bun run verify`, including `smoke:rule-fixtures` and `smoke:robustness`.
...
- Confirm the 导入报告 modal appears after loading, with object counts and compatibility findings. For `.geojson` and parcel indicator `.csv`, confirm it reports the matching adapter instead of UPF schema format errors.
