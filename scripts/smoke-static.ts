...
for (const token of ['方案综合评估', '权重敏感性分析', '案例验证包', '验证就绪度', 'UPF 结构校验报告', '规则目录与验证口径', '结构化 RuleSource', '用地兼容', 'DemoCanvasMetric', '导入报告', 'ArrowDown', '导入审计', 'service-radius-shape', '方案对比', '数据质量诊断', '结构化证据覆盖率', '不是可识别的 UPF 文件', '引用完整性保护']) {
