...
import { renderModalContent } from './markdown-renderer';
...
function auditImportedProject(input: UrbanPlanProject): ImportFinding[] {
    ...
    const auditObjects = Array.isArray(input.objects) ? input.objects : [];
...
function buildReport(): string {
    ...
    const lines = [
        ...
        '| 维度 | 权重 | 得分 | 解释 |',
        '|---|---:|---:|---|',
        ...evaluation.dimensions.map(dimension => markdownTableRow([dimension.name, `${(dimension.weight * 100).toFixed(0)}%`, dimension.score, dimension.reason])),
        '',
        '## 三、地块优先级',
        '',
        '| 地块 | 得分 | 状态 | 主要原因 |',
        ...
        '|---|---|---:|---:|',
        ...sensitivityRows.map(row => markdownTableRow([row.profile.name, row.winner?.scenario.name ?? '暂无', row.active?.evaluation.score ?? '-', row.spread])),
        '',
        '## 九、数据质量摘要',
        '',
        ...
        '| 项目 | 说明 |',
        '|---|---|',
        markdownTableRow(['UPF 版本', project.formatVersion]),
        markdownTableRow(['单位系统', `${UNIT_SYSTEM.name}，${UNIT_SYSTEM.metersPerCanvasUnit} 米/画布单位`]),
        markdownTableRow(['评价模型', evaluation.modelName]),
        markdownTableRow(['规则集', project.ruleset.version]),
        '',
...
function buildQualityReport(): string {
    ...
    if (importFindings.length) {
        lines.push(
            ...
            '| 等级 | 对象 | 问题 |',
            '|---|---|---|',
            ...importFindings.map(finding => markdownTableRow([finding.severity === 'warning' ? '警告' : '提示', finding.objectId, finding.message])),
        );
    } else {
...
function buildCaseValidationReport(): string {
    ...
    const lines = [
        ...
        '| 研究问题 | 系统证据 | 当前状态 |',
        '|---|---|---|',
        markdownTableRow(['RQ1：UPF 语义模型能否表达城市更新方案对象', `${project.objects.length} 个对象，${project.scenarios.length} 个情景方案`, quality.objectCount > 0 ? '可验证' : '需补数据']),
        markdownTableRow(['RQ2：规则校核能否形成可解释问题清单', `${checks.length} 条规则结果，${quality.ruleCatalog.length} 类规则触发`, errors.length ? '存在硬性风险' : '未发现硬性错误']),
        markdownTableRow(['RQ3：多方案比较能否支撑方案选择', `${decisionRows.length} 个方案进入决策矩阵`, decisionRows.length >= 2 ? '可比较' : '需增加对照方案']),
        markdownTableRow(['RQ4：评价结果对权重变化是否稳健', robustWinner ? `${robustWinner[0]} 获得 ${robustWinner[1]}/${EVALUATION_WEIGHT_PROFILES.length} 个模型第一` : '暂无稳健推荐', agreement >= 0.5 ? '可讨论稳健性' : '需说明价值偏好影响']),
        markdownTableRow(['RQ5：数据来源是否足以支撑论文答辩', `证据覆盖率 ${quality.evidenceCoverage.toFixed(1)}%，结构化覆盖率 ${quality.structuredEvidenceCoverage.toFixed(1)}%，质量分 ${quality.score}/100`, quality.score >= 80 ? '较充分' : '需补充来源']),
        '',
        '## 二、数据与对象概况',
        '',
        '| 数据项 | 当前值 | 论文写法 |',
        '|---|---:|---|',
        markdownTableRow(['地块', typeCounts.parcel ?? 0, '作为控规和更新评价的基本空间单元']),
        markdownTableRow(['道路', typeCounts.road ?? 0, '用于入口、通达性和道路等级复核']),
        markdownTableRow(['公共服务设施', typeCounts.facility ?? 0, '用于服务半径和配套能力分析']),
        markdownTableRow(['出入口', typeCounts.entrance ?? 0, '用于交通组织和界面关系检查']),
        markdownTableRow(['开放空间', typeCounts.openSpace ?? 0, '用于绿地率、公共开放空间和慢行连续性判断']),
        markdownTableRow(['约束控制线', typeCounts.constraint ?? 0, '用于历史风貌、蓝绿线、轨道保护等约束识别']),
        '',
        '### 证据类型分布',
        '',
        ...
        '|---|---:|---:|---:|---:|---:|---:|---:|---|',
        ...decisionRows.map(row => markdownTableRow([row.scenario.name, row.evaluation.score, row.evaluation.confidence, formatNumber(row.residents), formatNumber(row.residentialGfa), formatNumber(row.publicServiceGfa), row.errors, row.warnings, row.evaluation.band])),
        '',
        '## 五、权重敏感性与稳健性',
        '',
        ...
        '|---|---|---|---:|---:|',
        ...sensitivityRows.map(row => markdownTableRow([row.profile.name, row.profile.description, row.winner?.scenario.name ?? '暂无', row.active?.evaluation.score ?? '-', row.spread])),
        '',
        '## 六、规则校核与风险闭环',
        '',
        ...
        '|---|---|---|---|---|',
        ...(checks.length
            ? checks.map(check => markdownTableRow([severityLabel(check.severity), check.objectName, check.ruleId, `${check.title}：${check.message}`, check.source]))
            : ['| 通过 | 全局 | - | 当前没有规则问题 | - |']),
        '',
        '## 七、规则目录与验证口径',
        '',
        ...
        '|---|---|---|---|---:|---:|---|',
        ...RULE_CATALOG.map(rule => markdownTableRow([rule.id, rule.domain, severityLabel(rule.defaultSeverity), rule.source.level, rule.prototype ? '是' : '否', triggeredRuleIds.has(rule.id) ? '是' : '否', `${rule.source.title}；${rule.source.clause}`])),
        '',
        '## 八、论文实验记录表',
        '',
        '| 验证任务 | 操作对象 | 记录指标 | 结果填写 |',
...
function buildDecisionMatrixReport(): string {
    ...
    const lines = [
        ...
        '|---|---:|---:|---:|---:|---:|---:|---:|---|',
        ...rows.map(row => markdownTableRow([row.scenario.name, row.evaluation.score, row.evaluation.confidence, formatNumber(row.residents), formatNumber(row.residentialGfa), formatNumber(row.publicServiceGfa), row.errors, row.warnings, row.evaluation.band])),
        '',
        '## 二、推荐理由',
        '',
...
function buildWeightSensitivityReport(): string {
    ...
    const lines = [
        ...
        '|---|---|---|---:|---:|---:|',
        ...profileRows.map(row => markdownTableRow([row.profile.name, row.profile.description, row.winner?.scenario.name ?? '暂无', row.active?.evaluation.score ?? '-', row.winner?.evaluation.score ?? '-', row.spread])),
        '',
        '## 二、权重表',
        '',
        '| 模型 | 控规符合性 | 公共服务 | 交通组织 | 生态开放空间 | 更新价值 | 证据可信度 |',
        '|---|---:|---:|---:|---:|---:|---:|',
        ...EVALUATION_WEIGHT_PROFILES.map(profile => markdownTableRow([profile.name, asPercent(profile.weights.compliance), asPercent(profile.weights.publicService), asPercent(profile.weights.mobility), asPercent(profile.weights.ecology), asPercent(profile.weights.renewalValue), asPercent(profile.weights.evidence)])),
        '',
        '## 三、各模型排序',
        '',
        ...profileRows.flatMap(row => [
            ...
            '| 排名 | 方案 | 得分 | 状态 | 证据可信度 |',
            '|---:|---|---:|---|---:|',
            ...row.rows.map((item, index) => markdownTableRow([index + 1, item.scenario.name, item.evaluation.score, item.evaluation.band, item.evaluation.confidence])),
            '',
        ]),
        '## 四、答辩解释',
        '',
...
function buildImportReport(sourceName: string): string {
    ...
    const lines = [
        ...
        '| 类型 | 数量 |',
        '|---|---:|',
        markdownTableRow(['地块', counts.parcel ?? 0]),
        markdownTableRow(['道路', counts.road ?? 0]),
        markdownTableRow(['公共服务设施', counts.facility ?? 0]),
        markdownTableRow(['出入口', counts.entrance ?? 0]),
        markdownTableRow(['开放空间', counts.openSpace ?? 0]),
        markdownTableRow(['约束控制线', counts.constraint ?? 0]),
        '',
        '## 兼容与校验发现',
        '',
